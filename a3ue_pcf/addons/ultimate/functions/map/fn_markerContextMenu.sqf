...
// PCF: Upgrade Assets button parameter and marker type check
private _isUpgradeParamEnabled = (missionNamespace getVariable ["PCF_EnableResourceUpgrade", true]);
private _canShowUpgradePanel = _isUpgradeParamEnabled && {(_originalMarkerName in (resourcesX + factories))} && {_isPlayerControlled} && {!_isDestroyed};
...
// Reserve title bar space based on occupied slots
if (_canShowUpgradePanel) then {
    // Upgrade button sits in Slot 2, requiring 2 button units reserved from the right
    _titleWidth = _titleWidth - (2 * (_toggleBtnSize + _toggleBtnPadding));
} else {
    if (_canShowGarrisonPanel) then {
        // Only Garrison is visible in Slot 1, requiring 1 button unit reserved
        _titleWidth = _titleWidth - (_toggleBtnSize + _toggleBtnPadding);
    };
};

// PCF: The upgrade button space
if (_canShowUpgradePanel) then { 
    _titleWidth = _titleWidth - _toggleBtnSize - _toggleBtnPadding; 
};
...
if (_canShowUpgradePanel) then {
    ...
    _upgradeToggleBtn ctrlAddEventHandler ["ButtonClick", {
        ...
        // Toggle Upgrade panel off if already open
        private _upgradeGroup = _display getVariable ["A3U_mrkMenu_upgradeGrp", controlNull];
        if (!isNull _upgradeGroup) exitWith {
            ctrlDelete _upgradeGroup;
            _display setVariable ["A3U_mrkMenu_upgradeGrp", controlNull];
        };
        ...
        // Container Group
        private _panelGroup = _display ctrlCreate ["RscControlsGroupNoScrollbars", -1];
        _panelGroup ctrlSetPosition [_panelX, _panelY, _panelWidth, _panelHeight];
        _panelGroup ctrlCommit 0;
        _display setVariable ["A3U_mrkMenu_upgradeGrp", _panelGroup];
...
if (_originalMarkerName in citiesX && {!_isDestroyed}) then {
    ...
    if (!isNull _garrisonGroup) then { ctrlDelete _garrisonGroup; };
    if (!isNull _upgradeGroup) then { ctrlDelete _upgradeGroup; };
...
_closeButton ctrlAddEventHandler ["ButtonClick", {
    ...
    private _menuGroup = _display getVariable ["A3U_mrkMenu_grp", controlNull];
    private _garrisonGroup = _display getVariable ["A3U_mrkMenu_garrGrp", controlNull];
    private _upgradeGroup = _display getVariable ["A3U_mrkMenu_upgradeGrp", controlNull]; // PCF
    
    if (!isNull _menuGroup) then { ctrlDelete _menuGroup; };
    if (!isNull _garrisonGroup) then { ctrlDelete _garrisonGroup; };
    if (!isNull _upgradeGroup) then { ctrlDelete _upgradeGroup; }; // PCF

    _display setVariable ["A3U_mrkMenu_grp", controlNull];
    _display setVariable ["A3U_mrkMenu_garrGrp", controlNull];
    _display setVariable ["A3U_mrkMenu_upgradeGrp", controlNull]; // PCF
    _display setVariable ["A3U_mrkMenu_marker", ""];
}];