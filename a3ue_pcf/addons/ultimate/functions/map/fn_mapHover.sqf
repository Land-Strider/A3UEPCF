...
if (_attachTooltip) then {
    ...
    if (!isNull _menuGroup || {!isNull _garrisonGroup} || {!isNull _upgradeGroup}) then {
        ...
        // PCF: upgrade panel bounds check
        private _mouseInsideUpgrade = false;
        if (!isNull _upgradeGroup) then {
            private _pos = ctrlPosition _upgradeGroup;
            _mouseInsideUpgrade = (_mouseX >= _pos#0 && _mouseX <= _pos#0 + _pos#2 && _mouseY >= _pos#1 && _mouseY <= _pos#1 + _pos#3);
        };

        if (_mouseInsideMenu || {_mouseInsideGarrison} || {_mouseInsideUpgrade}) exitWith {};
