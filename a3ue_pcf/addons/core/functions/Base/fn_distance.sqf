...
private _processFIAMarker = {
    ...
    // PCF PCF_AAPostSpawnDistance parameter check start

    // Only distanceSPWN and distanceSPWN1 are modified for as they are used for occupants or invaders check. Player check remains unchanged.
    _distanceMultipliers = [1, 2, 3, 4, 5];  // corresponds to cases 1,2,3,4,5
    _multiplier = _distanceMultipliers select (PCF_AAPostSpawnDistance - 1);

    // Check if there are aircraft in the area for AA posts
    private _aaPostSpawnHasAircraftNearby = false;
    if (_marker in aapostsFIA) then {
        // Find all enemy units within the extended range and check if they are aircraft
        private _aircraftCheckDistance = distanceSPWN * _multiplier;
        private _enemyAirUnits = (allUnits inAreaArray [_position, _aircraftCheckDistance, _aircraftCheckDistance]) select {
            private _side = side group _x;
            (vehicle _x isKindOf "Air") and (_side == Occupants or _side == Invaders)
        };
        _aaPostSpawnHasAircraftNearby = !(_enemyAirUnits isEqualTo []);
    };

    // PCF distanceSPWN changed to distanceSPWN * _multiplier for AA posts when enemy aircraft are within the extended distance, unchanged for others
    _spawnDistance = if (_marker in aapostsFIA && _aaPostSpawnHasAircraftNearby) then { distanceSPWN * _multiplier } else { distanceSPWN };

    // PCF distanceSPWN1 changed to distanceSPWN * _multiplier (not distanceSPWN1 because this is already a larger distance to spawn or despawn at)... 
    // ...for AA posts when enemy aircraft are within the extended distance, but remain unchanged for others.
    _spawnDistance1 = if (_marker in aapostsFIA && _aaPostSpawnHasAircraftNearby) then { distanceSPWN * _multiplier } else { distanceSPWN1 };

    // PCF PCF_AAPostSpawnDistance parameter check end
    ...
    do
    {
        case ENABLED:
        {
			...
			// PCF distanceSPWN changed to _spawnDistance

            if (_occupants inAreaArray [_position, _spawnDistance, _spawnDistance] isNotEqualTo []
                || { _invaders inAreaArray [_position, _spawnDistance, _spawnDistance] isNotEqualTo []
        ...
        case DISABLED:
        {
            ...
            else
            {
                ...
                // PCF distanceSPWN1 changed to _spawnDistance1

                if (_occupants inAreaArray [_position, _spawnDistance1, _spawnDistance1] isNotEqualTo []
                    || { _invaders inAreaArray [_position, _spawnDistance1, _spawnDistance1] isNotEqualTo []
        ...
        case DESPAWN:
        {
            // Special distance for AA posts - 5x normal spawn distance
            ...
            if (_occupants inAreaArray [_position, _spawnDistance, _spawnDistance] isEqualTo []
            && { _invaders inAreaArray [_position, _spawnDistance, _spawnDistance] isEqualTo []
            && { _players inAreaArray [_position, distanceSPWN2, distanceSPWN2] isEqualTo []
            && { !(_marker in forcedSpawn)
            && !(_aaPostSpawnHasAircraftNearby) } }}) exitWith {};
