...
private _totalFactoryUpgrades = missionNamespace getVariable ["A3UE_TotalRebelFactoryUpgrades", 0];
private _totalUpgrades = missionNamespace getVariable ["A3UE_TotalRebelUpgrades", 0];

Info_1("Checking if marker %1 is a factory", _markerX);
if (_markerX in factories) then {
    // Find factory level in array
    private _factoryEntryIndex = _allFactoryLevels findIf {(_x select 0) isEqualTo _markerX};
    private _factoryLevel = if (_factoryEntryIndex >= 0) then {
        (_allFactoryLevels select _factoryEntryIndex) select 1
    } else {
        0
    };
    
    if ((_winner == teamPlayer) && (_loser != teamPlayer)) then {
        _totalFactoryUpgrades = _totalFactoryUpgrades + _factoryLevel;
        _totalUpgrades = _totalUpgrades + _factoryLevel;
    } else {
        if ((_winner != teamPlayer) && (_loser == teamPlayer)) then {
            _totalFactoryUpgrades = (_totalFactoryUpgrades - _factoryLevel) max 0;
            _totalUpgrades = (_totalUpgrades - _factoryLevel) max 0;
            
            // Reset lost factory's upgrade level
            private _currentFactoryLevel = if (_factoryEntryIndex >= 0) then {
                (_allFactoryLevels select _factoryEntryIndex) select 1
            } else {
                0
            };
            _currentFactoryLevel = (_currentFactoryLevel - 1) max 0;
            
            // Update array entry
            if (_factoryEntryIndex >= 0) then {
                (_allFactoryLevels select _factoryEntryIndex) set [1, _currentFactoryLevel];
            } else {
                _allFactoryLevels pushBack [_markerX, _currentFactoryLevel];
            };
            
            missionNamespace setVariable ["A3UE_FactoryLevels", _allFactoryLevels, true];
            missionNamespace setVariable [format ["A3UE_FactoryUpgradeLevel_%1", _markerX], _currentFactoryLevel, true];
        };
    };
    missionNamespace setVariable ["A3UE_TotalRebelFactoryUpgrades", _totalFactoryUpgrades, true];
    missionNamespace setVariable ["A3UE_TotalRebelUpgrades", _totalUpgrades, true];
};

// --- Update Rebel Resource Upgrades ---
private _allResourceLevels = missionNamespace getVariable ["A3UE_ResourceLevels", []];
private _totalResourceUpgrades = missionNamespace getVariable ["A3UE_TotalRebelResourceUpgrades", 0];
private _totalUpgrades = missionNamespace getVariable ["A3UE_TotalRebelUpgrades", 0];

Info_1("Checking if marker %1 is a resource", _markerX);
if (_markerX in resourcesX) then {
    // Find resource level in array
    private _resourceEntryIndex = _allResourceLevels findIf {(_x select 0) isEqualTo _markerX};
    private _resourceLevel = if (_resourceEntryIndex >= 0) then {
        (_allResourceLevels select _resourceEntryIndex) select 1
    } else {
        0
    };
    
    if ((_winner == teamPlayer) && (_loser != teamPlayer)) then {
        _totalResourceUpgrades = _totalResourceUpgrades + _resourceLevel;
        _totalUpgrades = _totalUpgrades + _resourceLevel;
    } else {
        if ((_winner != teamPlayer) && (_loser == teamPlayer)) then {
            _totalResourceUpgrades = (_totalResourceUpgrades - _resourceLevel) max 0;
            _totalUpgrades = (_totalUpgrades - _resourceLevel) max 0;
            
            // Reset lost resource's upgrade level
            private _currentResourceLevel = if (_resourceEntryIndex >= 0) then {
                (_allResourceLevels select _resourceEntryIndex) select 1
            } else {
                0
            };
            _currentResourceLevel = (_currentResourceLevel - 1) max 0;
            
            // Update array entry
            if (_resourceEntryIndex >= 0) then {
                (_allResourceLevels select _resourceEntryIndex) set [1, _currentResourceLevel];
            } else {
                _allResourceLevels pushBack [_markerX, _currentResourceLevel];
            };
            
            missionNamespace setVariable ["A3UE_ResourceLevels", _allResourceLevels, true];
            missionNamespace setVariable [format ["A3UE_ResourceUpgradeLevel_%1", _markerX], _currentResourceLevel, true];
        };
    };
    missionNamespace setVariable ["A3UE_TotalRebelResourceUpgrades", _totalResourceUpgrades, true];
    missionNamespace setVariable ["A3UE_TotalRebelUpgrades", _totalUpgrades, true];
};

// Update marker display to reflect new upgrade levels
