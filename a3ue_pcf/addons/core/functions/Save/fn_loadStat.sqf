    ...
    switch (_varName) do {
        ...
        case 'watchpostsFIA': {
			if (count (_varValue select 0) == 2) then {
			{
                    ...
                    // PCF Short Emplacement Parameter tie-in start
                    if (PCF_EmplacementShortName) then {
                        _mrk setMarkerText format [localize "STR_a3ue_pcf_marker_watchpost_short"];
                    } else {
                        _mrk setMarkerText format [localize "STR_marker_watchpost",FactionGet(reb,"name")]; // Antistasi Default
                    };
                    // PCF Short Emplacement Parameter tie-in end
		...
		case 'roadblocksFIA': {
			if (count (_varValue select 0) == 2) then {
			{
                    ...
                    // PCF Short Emplacement Parameter tie-in start
                    if (PCF_EmplacementShortName) then {
                        _mrk setMarkerText format [localize "STR_a3ue_pcf_marker_roadblock_short"];
                    } else {
                        _mrk setMarkerText format [localize "STR_marker_roadblock",FactionGet(reb,"name")]; // Antistasi Default
                    };
                    // PCF Short Emplacement Parameter tie-in end
					spawner setVariable [_mrk,2,true];
					if (count _garrison > 0) then {garrison setVariable [_mrk,_garrison,true]};
		...
		case 'aapostsFIA': {
			if (count (_varValue select 0) >= 2) then {
			{
                    ...
                    // PCF Short Emplacement Parameter tie-in start
                    if (PCF_EmplacementShortName) then {
                        _mrk setMarkerText format [localize "STR_a3ue_pcf_marker_aa_empl_short"];
                    } else {
                        _mrk setMarkerText format [localize "STR_marker_aa_empl",FactionGet(reb,"name")]; // Antistasi Default
                    };
                    // PCF Short Emplacement Parameter tie-in end
					spawner setVariable [_mrk,2,true];
					if (count _garrison > 0) then {garrison setVariable [_mrk,_garrison,true]};
		...
		case 'atpostsFIA': {
			if (count (_varValue select 0) >= 2) then {
			{
                    ...
                    // PCF Short Emplacement Parameter tie-in start
                    if (PCF_EmplacementShortName) then {
                        _mrk setMarkerText format [localize "STR_a3ue_pcf_marker_at_empl_short"];
                    } else {
                        _mrk setMarkerText format [localize "STR_marker_at_empl", FactionGet(reb,"name")]; // Antistasi Default
                    };
                    // PCF Short Emplacement Parameter tie-in end
					spawner setVariable [_mrk,2,true];
					if (count _garrison > 0) then {garrison setVariable [_mrk,_garrison,true]};
		...
		case 'hmgpostsFIA': {
			if (count (_varValue select 0) >= 2) then {
			{
                    ...
                    // PCF Short Emplacement Parameter tie-in start
                    if (PCF_EmplacementShortName) then {
                        _mrk setMarkerText format [localize "STR_a3ue_pcf_marker_hmg_empl_short"];
                    } else {
                        _mrk setMarkerText format [localize "STR_marker_hmg_empl", FactionGet(reb,"name")]; // Antistasi Default
                    };
                    // PCF Short Emplacement Parameter tie-in end
					spawner setVariable [_mrk,2,true];
					if (count _garrison > 0) then {garrison setVariable [_mrk,_garrison,true]};
