...
if (side _injurer == teamPlayer) then
{
	if (_part == "") then
	{
		if (_damage >= 1) then
		{
			...
			{
				// NEW: Add chance-based unconscious trigger
				private _unconsciousChance = [1,100] call BIS_fnc_randomNum;
				if (_unconsciousChance <= PCF_enemyUnconsciousChance) then
				{
					_damage = 0.9;
				...
				{
					// Enemy dies instead of becoming unconscious
		...
		if (_damage >= 1) then
		{
			if !(_part in ["arms","hands","legs"]) then
			{
				...
				if (_part in ["head","body"]) then
				{
					if !(_unit getVariable ["incapacitated",false] && {_unit getVariable ["canBeIncapacitated",true]}) then
					{
						// NEW: Add chance-based unconscious trigger for body parts
						private _unconsciousChance = [50,100] call BIS_fnc_randomNum;
						if (_unconsciousChance <= PCF_enemyUnconsciousChance) then
						{
							[_unit,_injurer] call _makeUnconscious;
						}
